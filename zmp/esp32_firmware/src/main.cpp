...
void setup() {
    ...
    delay(2000);
    Serial.println("ESP32-S3 HUNO Controller (GankenKun Port)");
    printHelp();
    
    motion.begin(); // init LittleFS
    walkEngine.begin();

    // Create RTOS Task for Walking on Core 1 (app_cpu)
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        server.begin();
        Serial.println("HTTP server started on Core 0");
    } else {
...
void loop() {
    // Core 1 runs walkTask
    // Core 0 (default loop) runs CLI processing and WebSerial
